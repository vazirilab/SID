function exit_code = main_nnmf_SID(config_in)
%% SID main function. 
...
% if Input.bg_sub
%     [SID_output.bg_temporal, SID_output.bg_spatial] = par_rank_1_factorization(Input.indir, Input.frames.step, Input.bg_iter, 0, 0, 0, 0, Input.frames.end, Input.mask);
% else
...
% if Input.rectify
%     [SID_output.std_image, ~] = par_compute_std_image(Input.indir, Input.frames.step, SID_output.bg_temporal, SID_output.bg_spatial_pre_crop_rectify, ...
...
if Input.recon_final_spatial_filters
    ...
    %[SID_output.recon_NSF, ~, ~, ~] = fast_NSF_recon(forward_model, ceil(SID_output.neuron_centers_iterated), psf_ballistic, size(SID_output.std_image));
