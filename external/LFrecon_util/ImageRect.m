function IMG_RESAMPLE_crop2 = ImageRect(IMG_BW, xCenter, yCenter, dx, M, Crop, XcutLeft, XcutRight, YcutUp, YcutDown, gpu_device)
if nargin < 11
    gpu_device = false;
end
...
IMG_RESAMPLE = gather(interp2(X, Y, IMG_BW, Xqq, Yqq));
IMG_RESAMPLE = IMG_RESAMPLE( (1:1:M*floor((size(IMG_RESAMPLE,1)-YqInit)/M)), (1:1:M*floor((size(IMG_RESAMPLE,2)-XqInit)/M)) );
